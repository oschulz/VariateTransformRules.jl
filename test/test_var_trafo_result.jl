...
#using VariateTransformRules: var_trafo_result #, var_trafo_ladj
...
@testset "var_trafo_result" begin
    function test_back_and_forth(trg_d, src_d)
        @testset "transform $(typeof(trg_d).name) <-> $(typeof(src_d).name)" begin
            ...
            @test @inferred(VariateTransformRules.apply_dist_trafo(trg_d, src_d, src_v, prev_ladj)) isa NamedTuple{(:v,:ladj)}
            trg_v, trg_ladj = VariateTransformRules.apply_dist_trafo(trg_d, src_d, src_v, prev_ladj)

            @test VariateTransformRules.apply_dist_trafo(src_d, trg_d, trg_v, trg_ladj) isa NamedTuple{(:v,:ladj)}
            src_v_reco, prev_ladj_reco = VariateTransformRules.apply_dist_trafo(src_d, trg_d, trg_v, trg_ladj)

            @test src_v ≈ src_v_reco
            @test prev_ladj ≈ prev_ladj_reco
            @test trg_ladj ≈ logabsdet(ForwardDiff.jacobian(x -> unshaped(VariateTransformRules.apply_dist_trafo(trg_d, src_d, x, prev_ladj).v), src_v))[1] + prev_ladj
        end
    end

    function get_trgxs(trg_d, src_d, X)
        return (x -> VariateTransformRules.apply_dist_trafo(trg_d, src_d, x, 0.0).v).(nestedview(X))
    end

    function get_trgxs(trg_d, src_d::Distribution{Univariate}, X)
        return (x -> VariateTransformRules.apply_dist_trafo(trg_d, src_d, x, 0.0).v).(X)
    end
    ...
    uvnorm = VariateTransformRules.StandardUvNormal()

    standnorm1 = VariateTransformRules.StandardMvNormal(1)
    standnorm2 = VariateTransformRules.StandardMvNormal(2)

    standuni2 = VariateTransformRules.StandardMvUniform(2)
    ...
    test_back_and_forth(ntdist, VariateTransformRules.StandardMvNormal(5))
    test_back_and_forth(ntdist, VariateTransformRules.StandardMvUniform(5))

    let
        mvuni = product_distribution([Uniform(), Uniform()])

        x = rand()
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, mvnorm, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, standnorm1, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, standnorm2, x, 0.0)

        x = rand(2)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(mvuni, mvnorm, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(mvnorm, mvuni, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, mvnorm, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, standnorm1, x, 0.0)
        @test_throws ArgumentError VariateTransformRules.apply_dist_trafo(uvnorm, standnorm2, x, 0.0)
    end
    ...
    let
        ...
        src_d = VariateTransformRules.StandardMvNormal(totalndof(varshape(trg_d)))
    ...
    @testset "Custom cdf and quantile for dual numbers" begin
        ...
        @test VariateTransformRules._trafo_cdf(Normal(Dual(0, 1, 0, 0), Dual(1, 0, 1, 0)), Dual(0.5, 0, 0, 1)) == cdf(Normal(Dual(0, 1, 0, 0), Dual(1, 0, 1, 0)), Dual(0.5, 0, 0, 1))
        @test VariateTransformRules._trafo_cdf(Normal(0, 1), Dual(0.5, 1)) == cdf(Normal(0, 1), Dual(0.5, 1))

        @test VariateTransformRules._trafo_quantile(Normal(0, 1), Dual(0.5, 1)) == quantile(Normal(0, 1), Dual(0.5, 1))
        @test VariateTransformRules._trafo_quantile(Normal(Dual(0, 1, 0, 0), Dual(1, 0, 1, 0)), Dual(0.5, 0, 0, 1)) == quantile(Normal(Dual(0, 1, 0, 0), Dual(1, 0, 1, 0)), Dual(0.5, 0, 0, 1))
    end
    ...
    @testset "trafo autodiff pullbacks" begin
        ...
        f = inv(VariateTransformRules.DistributionTransform(Uniform, DistributionsAD.TuringDirichlet([3.0, 4.0, 5.0, 6.0, 7.0])))
        @test isapprox(ForwardDiff.jacobian(f, src_v), Zygote.jacobian(f, src_v)[1], rtol = 10^-4)
        f = inv(VariateTransformRules.DistributionTransform(Uniform, Dirichlet([3.0, 4.0, 5.0, 6.0, 7.0])))
        @test isapprox(ForwardDiff.jacobian(f, src_v), Zygote.jacobian(f, src_v)[1], rtol = 10^-4)
        f = inv(VariateTransformRules.DistributionTransform(Normal, Dirichlet([3.0, 4.0, 5.0, 6.0, 7.0])))
        @test isapprox(ForwardDiff.jacobian(f, src_v), Zygote.jacobian(f, src_v)[1], rtol = 10^-4)
    end
end
